...
public class SettingTabTest extends TestInitializer {
...
@Test(priority=1)
public void testValidatefleetManagementSettingsAndHMC(String browser,String passPortUSer){
		...
		fleetManagementPage.clickSettingsTab(wd);
		stemOfMes=fleetManagementPage.selectSystemofMeasurement();
		fleetManagementPage.clickSettingSave();
		fleetManagementPage.getSelectedSystemOfOpt();
		/*try {
			wd = WebdriverSelector.getDriver(wd, browser);
