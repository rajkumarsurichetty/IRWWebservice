...
public class SelectOrgSearchResultZero extends TestInitializer {
	...
	@Test(priority=1,dataProvider="SelectOrgSearchResultZero")
	...
	testCaseNumberID=testCaseNumber;
		...
		selectOrgPage.clickSearchButton();
		selectOrgPage.verifyOrganizationSearchReasultsZero(wd, custNo);
	...
	@DataProvider(name="SelectOrgSearchResultZero")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("SelectOrgSearchResultZero");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
			ReportGenerator.setLogAndCreateScreenshot("SelectOrgSearchResultZero~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
			//addResultForTestCase("4603", "1350059", 1,"Test Passed  "+i++ );+ className + "_" + testCaseName + "_" + testData + "_"+ status +
			addResultForTestCase(testRunId,testCaseNumberID,1, "Select Organization Results Zero Test~ "+browserName+"__"+localTestData+"_"+Constants.PASSED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.PASSED+".jpg)");
			isBrowserAlive="N";
			wd.quit();		
		} else 
		{//http://localhost/Hybris/Screenshots/
			ReportGenerator.setLogAndCreateScreenshot("SelectOrgSearchResultZero~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
