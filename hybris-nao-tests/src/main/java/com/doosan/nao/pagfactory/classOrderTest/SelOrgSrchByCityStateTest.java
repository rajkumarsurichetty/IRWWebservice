...
public class SelOrgSrchByCityStateTest extends TestInitializer {
	...
	@Test(priority=1,dataProvider="SelOrgSrchByCityStateTest")
	public void validateSelectOrgPageTwoDropDownTest(String testData,String userName,String password, String brand, String custNo, String selectOptVal, String qty, String binLocation, String comments, String testUrlName,String testCaseNumber) throws InterruptedException
	{	testRunId=PROJECT.getProperty("testRunID");
		...
		String [] optValueNTest=selectOptVal.split(",");
		selectOrgPage.selectOptionFirstdropdown(wd, optValueNTest[0]);
		selectOrgPage.enterTextFirstContainsBox(optValueNTest[1]);
		selectOrgPage.clickSearchButton();
		selectOrgPage.verifyCitySearchReasults(wd,optValueNTest[1]);
	...
	@DataProvider(name="SelOrgSrchByCityStateTest")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("SelOrgSrchByCityStateTest");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
			ReportGenerator.setLogAndCreateScreenshot("SelOrgSrchByCityStateTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
			//addResultForTestCase("4603", "1350059", 1,"Test Passed  "+i++ );+ className + "_" + testCaseName + "_" + testData + "_"+ status +
			addResultForTestCase(testRunId,testCaseNumberID,1, "Select Organization elements Select Org and Search by city or state Test~ "+browserName+"__"+localTestData+"_"+Constants.PASSED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.PASSED+".jpg)");
			isBrowserAlive="N";
			wd.quit();		
		} else 
		{//http://localhost/Hybris/Screenshots/
			ReportGenerator.setLogAndCreateScreenshot("SelOrgSrchByCityStateTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
			addResultForTestCase(testRunId,testCaseNumberID,5, "Select Organization elements Select Org and Search by city or state test failed ~ "+browserName+"__"+localTestData+"_"+Constants.PASSED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.PASSED+".jpg)");
