...
public class SelectOrgIsOptionDisabledTest extends TestInitializer {
	...
	@Test(priority=1,dataProvider="SelectOrgIsOptionDisabledTest")
	...
	{	testRunId=PROJECT.getProperty("testRunID");
		...
		selectOrgPage.verifyOptionisDisabled2Drp(wd);
		selectOrgPage.getSelectedOptionFrmSecondDropDown(wd);
		selectOrgPage.verifyOptionisDisabled1Drp(wd);
	...
	@DataProvider(name="SelectOrgIsOptionDisabledTest")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("SelectOrgIsOptionDisabledTest");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
			ReportGenerator.setLogAndCreateScreenshot("SelectOrgIsOptionDisabledTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
			//addResultForTestCase("4603", "1350059", 1,"Test Passed  "+i++ );+ className + "_" + testCaseName + "_" + testData + "_"+ status +
			addResultForTestCase(testRunId,testCaseNumberID, 1, "Select Organization Slected option is disabled in 2ed dropdown ~ "+browserName+"__"+localTestData+"_"+Constants.PASSED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.PASSED+".jpg)");
			isBrowserAlive="N";
			wd.quit();		
		} else 
		{//http://localhost/Hybris/Screenshots/
			ReportGenerator.setLogAndCreateScreenshot("SelectOrgIsOptionDisabledTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
			addResultForTestCase(testRunId,testCaseNumberID, 5, "Select Organization Slected option is not disabled in 2ed dropdown ~ "+browserName+"__"+localTestData+"_"+Constants.FAILED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.FAILED+".jpg)"+result.getThrowable());
