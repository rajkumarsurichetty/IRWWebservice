...
public class QuickOrdVerifyEmptyTableAddLine extends TestInitializer{
	...
	@Test(priority=1,dataProvider="QuickOrdVerifyEmptyTableAddLine")
	...
	{	testRunId=PROJECT.getProperty("testRunID");
		...
		quickOrderPage.verifyAddlineErrorisDispaly();
	...
	@DataProvider(name="QuickOrdVerifyEmptyTableAddLine")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("QuickOrdVerifyEmptyTableAddLine");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
		path=ReportGenerator.setLogAndCreateScreenshot("QuickOrdVerifyEmptyTableAddLine~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
		...
		{//http://localhost/Hybris/Screenshots/
			path=ReportGenerator.setLogAndCreateScreenshot("QuickOrdVerifyEmptyTableAddLine~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
