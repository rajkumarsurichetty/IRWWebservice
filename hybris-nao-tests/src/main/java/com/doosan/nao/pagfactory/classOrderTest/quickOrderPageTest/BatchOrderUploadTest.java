...
public class BatchOrderUploadTest extends TestInitializer{
	...
	@Test(priority=1,dataProvider="BatchOrderUploadTest")
	...
	{	testRunId=PROJECT.getProperty("testRunID");
		...
		try {
			quickOrderPage.bathcOrderUploadTest(testData, itemNum, qty, binLocation, comments, wd);
		} catch (IOException e) {
	...
	@DataProvider(name="BatchOrderUploadTest")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("BatchOrderUploadTest");
