...
public class QuickOrderPageUIElementsTest extends TestInitializer {
	...
	@Test(priority=1,dataProvider="QuickOrderPageUIElementsTest")
	...
	@DataProvider(name="QuickOrderPageUIElementsTest")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("QuickOrderPageUIElementsTest");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
		path=ReportGenerator.setLogAndCreateScreenshot("QuickOrderPageUIElementsTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
		...
		{//http://localhost/Hybris/Screenshots/
			path=ReportGenerator.setLogAndCreateScreenshot("QuickOrderPageUIElementsTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
