...
import com.doosan.nao.pagfactory.classOrderTest.Testing;
import com.doosan.nao.reports.ReportGenerator;

public class SPSaveToCartTest extends TestInitializer {
	...
	{ 	System.out.println(" Verify the Shopping cart Page UI Objects ");
		browserName=browser;
		if (isBrowserAlive=="N")
		{
	...
	@Test(priority=1,dataProvider="SPSaveToCartTest")
	public void validateSaveToCartTest(String testData,String userName,String password, String brand, String custNo, String itemNum, String qty, String binLocation, String comments, String testUrlName,String testCaseNumber) throws InterruptedException
	{	testRunId=PROJECT.getProperty("testRunID");
		...
		yourShoppingCartPage.getItemQtyfromShoppingGrid();
		
		yourShoppingCartPage.getItemBinfromShoppingGrid();
		
		yourShoppingCartPage.getItemCommfromShoppingGrid();
		
		yourShoppingCartPage.clicksaveToSavedCartsButton(wd);
		yourShoppingCartPage.verifySaveCartHeaderText(wd);
				
	}
		

	
	@DataProvider(name="SPSaveToCartTest")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("SPSaveToCartTest");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
		path=ReportGenerator.setLogAndCreateScreenshot("SPSaveToCartTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
			//addResultForTestCase("4603", "1350059", 1,"Test Passed  "+i++ );+ className + "_" + testCaseName + "_" + testData + "_"+ status +
			addResultForTestCase(testRunId,testCaseNumberID,1, "Save Cart  Test~ "+browserName+"__"+localTestData+"_"+Constants.PASSED+"\n"+"![](http://che-dt-idc1/Hybris/reports/Screenshots/"+path+")");
		...
		{//http://localhost/Hybris/Screenshots/
			path=ReportGenerator.setLogAndCreateScreenshot("SPSaveToCartTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
			addResultForTestCase(testRunId,testCaseNumberID,5, "Save cart  Test failed ~ "+browserName+"__"+localTestData+"_"+Constants.FAILED+"\n"+result.getThrowable()+"![](http://che-dt-idc1/Hybris/reports/Screenshots/"+path+")");
