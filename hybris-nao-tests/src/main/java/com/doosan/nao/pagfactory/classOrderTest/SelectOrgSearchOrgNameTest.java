...
public class SelectOrgSearchOrgNameTest extends TestInitializer {
	...
	@Test(priority=1,dataProvider="SelectOrgSearchOrgNameTest")
	...
	{	testRunId=PROJECT.getProperty("testRunID");
		...
		selectOrgPage.enterTextFirstContainsBox(OrgName);
		selectOrgPage.clickSearchButton();
		selectOrgPage.verifyOrganizationSearchReasults(wd,OrgName);
	...
	@DataProvider(name="SelectOrgSearchOrgNameTest")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("SelectOrgSearchOrgNameTest");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
			ReportGenerator.setLogAndCreateScreenshot("SelectOrgSearchOrgNameTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
			//addResultForTestCase("4603", "1350059", 1,"Test Passed  "+i++ );+ className + "_" + testCaseName + "_" + testData + "_"+ status +
			addResultForTestCase(testRunId,testCaseNumberID,1, "Select Organization search Results by Name Test~ "+browserName+"__"+localTestData+"_"+Constants.PASSED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.PASSED+".jpg)");
			isBrowserAlive="N";
			wd.quit();		
		} else 
		{//http://localhost/Hybris/Screenshots/
			ReportGenerator.setLogAndCreateScreenshot("SelectOrgSearchOrgNameTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
			addResultForTestCase(testRunId,testCaseNumberID,5, "Select Organization Search Results by Name failed ~ "+browserName+"__"+localTestData+"_"+Constants.FAILED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.FAILED+".jpg)");
