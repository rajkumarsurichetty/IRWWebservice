...
public class SelectOrgPageUITest extends TestInitializer{
	...
	@BeforeMethod
	public void setUpForSelectOrgPageUITest(String browser, String browserVersion, String platform, String osVersion) throws IOException, URISyntaxException, InterruptedException
	{ 	
	...
	@Test(priority=1,dataProvider="SelectOrgPageUITest")
	public void validateSelectOrgPageUITest(String testData,String userName,String password, String brand, String custNo, String itemNo, String qty, String binLocation, String comments, String testUrlName,String testCaseNumber) throws InterruptedException
	...
	testCaseNumberID=testCaseNumber;
		...
		selectOrgPage.verifyFirstContainsTextBox_isplayed(wd);
		selectOrgPage.verifyBySecondDropDown(wd);
		selectOrgPage.verifyBySecondcontainsTextBox(wd);
		selectOrgPage.verifySearchButton(wd);
		
		selectOrgPage.verifyResultsperpageDropDown(wd);
		selectOrgPage.verifyShowingResultsIsDispaly(wd);
		selectOrgPage.verifyFistPagenateButtonIsDispaly(wd);
		selectOrgPage.verifyPreviousPagenateButtonIsDispaly(wd);
		selectOrgPage.verifyNextPagenateButtonIsDispaly(wd);
		selectOrgPage.verifyLastPagenateButtonIsDispaly(wd);
		
		selectOrgPage.verifyOrganizationListTableIsDisplay(wd);
		selectOrgPage.verifyOrganizationListTableHeaderIsDisplay(wd);
		selectOrgPage.verifyOrganizationListTableBodyIsDisplay(wd);
		selectOrgPage.verifyOrganizationListTableBodyDataIsDisplay(wd);
		//selectOrgPage.verifyOrganizationListTableBodyDataIsEmpty(wd);
	...
	@DataProvider(name="SelectOrgPageUITest")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("SelectOrgPageUITest");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
			ReportGenerator.setLogAndCreateScreenshot("SelectOrgPageUITest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
			//addResultForTestCase("4603", "1350059", 1,"Test Passed  "+i++ );+ className + "_" + testCaseName + "_" + testData + "_"+ status +
			addResultForTestCase(testRunId,testCaseNumberID, 1, "Select Organization UI elements SelectOrgPageUITest~ "+browserName+"__"+localTestData+"_"+Constants.PASSED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.PASSED+".jpg)");
			isBrowserAlive="N";
			wd.quit();		
		} else 
		{//http://localhost/Hybris/Screenshots/
			ReportGenerator.setLogAndCreateScreenshot("SelectOrgPageUITest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
			addResultForTestCase(testRunId,testCaseNumberID, 5, "Select Organization UI elements SelectOrgPageUITest~ "+browserName+"__"+localTestData+"_"+Constants.PASSED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.PASSED+".jpg)");
