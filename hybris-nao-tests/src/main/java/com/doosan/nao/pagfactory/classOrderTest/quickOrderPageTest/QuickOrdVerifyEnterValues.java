...
public class QuickOrdVerifyEnterValues extends TestInitializer{
	...
	@Test(priority=1,dataProvider="QuickOrdVerifyEnterValues")
	...
	@DataProvider(name="QuickOrdVerifyEnterValues")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("QuickOrdVerifyEnterValues");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
		path=ReportGenerator.setLogAndCreateScreenshot("QuickOrdVerifyEnterValues~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
		...
		{//http://localhost/Hybris/Screenshots/
			path=ReportGenerator.setLogAndCreateScreenshot("QuickOrdVerifyEnterValues~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
