...
public class SPCartItemsAddCheckTest extends TestInitializer {
	...
	@Test(priority=1,dataProvider="SPCartItemsAddCheckTest")
	...
	@DataProvider(name="SPCartItemsAddCheckTest")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("SPCartItemsAddCheckTest");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
		path=ReportGenerator.setLogAndCreateScreenshot("SPCartItemsAddCheckTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
		...
		{//http://localhost/Hybris/Screenshots/
			path=ReportGenerator.setLogAndCreateScreenshot("SPCartItemsAddCheckTest~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
