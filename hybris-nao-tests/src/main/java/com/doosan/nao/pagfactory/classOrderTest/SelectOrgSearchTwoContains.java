...
public class SelectOrgSearchTwoContains extends TestInitializer {
	...
	@Test(priority=1,dataProvider="SelectOrgSearchTwoContains")
	...
	@DataProvider(name="SelectOrgSearchTwoContains")
	public static Object[][] getDataForShipments() throws InterruptedException
	{
		//TestInitializer.isBrowserAlive="N";
		return InputReader.getInputReader().getDataFromXLS("SelectOrgSearchTwoContains");
	...
	{int i=0;
		...
		{
			isBrowserAlive="A";
			ReportGenerator.setLogAndCreateScreenshot("SelectOrgSearchTwoContains~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.PASSED, wd);
			//addResultForTestCase("4603", "1350059", 1,"Test Passed  "+i++ );+ className + "_" + testCaseName + "_" + testData + "_"+ status +
			addResultForTestCase(testRunId,testCaseNumberID,1, "Select Organization search Results by Two contains  by Customer and name Test~ "+browserName+"__"+localTestData+"_"+Constants.PASSED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.PASSED+".jpg)");
			isBrowserAlive="N";
			wd.quit();		
		} else 
		{//http://localhost/Hybris/Screenshots/
			ReportGenerator.setLogAndCreateScreenshot("SelectOrgSearchTwoContains~"+browserName, Constants.DEFAULT_TESTNAME, localTestData, Constants.FAILED,result.getThrowable().toString(),wd);
			addResultForTestCase(testRunId,testCaseNumberID,5, "Select Organization ResultsZero test failed ~ "+browserName+"__"+localTestData+"_"+Constants.FAILED+"\n"+"![](http://localhost/HybrisAWSCons/SelectOrgPageUITest~"+browserName+"__"+localTestData+"_"+Constants.FAILED+".jpg)");
